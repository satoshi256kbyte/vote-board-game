...
- Specのフォルダには連番をつけてください
  - {連番}-Spec名
- Specの各ファイルは必ず日本語で書いてください

### タスク完了条件（tasks.md の個別タスク単位）

tasks.md の各タスク（例: `1.1 ○○を実装`）ごとに、以下のサイクルを回すこと:

```text
タスク着手（in_progress）
  → コード実装
  → pnpm test（全テスト実行）
    → fail あり → 修正 → 再度 pnpm test → 全 pass するまで繰り返し
    → 全 pass → git commit → git push → タスク完了（completed）
```

ルール:

1. `pnpm test` が全 pass しない限り、タスクを完了にしてはならない
2. 自分が変更したコード以外のテストが fail した場合も、原因を調査して修正すること（既存テストを壊さない）
3. 新規コードには対応するテストを追加すること
4. コミットとプッシュはタスク単位で行う（複数タスクをまとめない）
5. コミットメッセージにはタスク番号を含める（例: `feat: 1.1 ユーザー認証を実装`）
...
heredoc を使う場合はタイムアウトを設定して一定時間で編集が終わらない場合はコマンドが終了するようにしてください。不完全なコマンドとなり編集が無限に止まる可能性があります。
...
E2E テスト（Playwright）の作成・編集・デバッグ時には、以下の MCP サーバーを活用すること:

| MCP サーバー | 用途                                                             |
| ------------ | ---------------------------------------------------------------- |
| `playwright` | ブラウザ操作（ページ遷移、クリック、入力、スナップショット取得） |
| `context7`   | Playwright の最新 API ドキュメント・コード例の検索               |

#### 具体的な使用場面

- **E2E テストの動作確認・デバッグ時**: `playwright` MCP サーバーでブラウザを操作し、実際の画面遷移や要素の状態を確認する
  - `browser_navigate` でテスト対象のページに遷移
  - `browser_snapshot` でページのアクセシビリティツリーを取得し、セレクタやロールを確認
  - `browser_click`、`browser_type`、`browser_fill_form` で操作を再現
  - `browser_take_screenshot` で視覚的な状態を確認
  - `browser_console_messages` でコンソールエラーを確認
  - `browser_network_requests` でAPI通信の状態を確認
- **Playwright の API や使い方が不明な時**: `context7` で Playwright のドキュメントを検索する
- **テストが失敗する原因調査時**: `playwright` MCP サーバーで実際にブラウザを操作し、要素の存在やページの状態を確認する
