...
- [x] 6. 候補詳細用 OGP 画像 API の改修
  - [x] 6.1 `packages/web/src/app/api/og/candidate/[candidateId]/route.tsx` を改修する
  ...
  - [x] 6.2 `packages/web/src/app/api/og/candidate/[candidateId]/route.test.tsx` のユニットテストを更新する
    - 正常時の画像生成（ハイライト付き盤面）、データ取得失敗時のフォールバックを検証
    - _要件: 1.2, 1.6_

- [x] 7. チェックポイント - OGP 画像 API の検証
  - すべてのテストが pass することを確認し、ユーザーに質問があれば確認する。

- [x] 8. ShareButton コンポーネントの拡張
  - [x] 8.1 `packages/web/src/components/share-button.tsx` を改修する
  ...
  - [x] 8.2 `packages/web/src/components/share-button.test.tsx` のユニットテストを更新する
...
- [x] 9. 対局詳細画面のメタデータ生成
  - [x] 9.1 `packages/web/src/app/games/[gameId]/layout.tsx` を新規作成する
  ...
  - [x] 9.2 `packages/web/src/app/games/[gameId]/layout.test.tsx` にユニットテストを作成する
...
- [x] 10. 特定ターン画面の新規作成
  - [x] 10.1 `packages/web/src/app/games/[gameId]/turns/[turnNumber]/page.tsx` を新規作成する
  ...
  - [x] 10.2 `packages/web/src/app/games/[gameId]/turns/[turnNumber]/page.test.tsx` にユニットテストを作成する
    - `generateMetadata` が正しいメタデータを返すことを検証
    - 盤面と ShareButton が表示されることを検証
    - _要件: 5.1, 5.2, 5.3, 5.4_

- [x] 11. 候補詳細画面のメタタグ改善
  - [x] 11.1 `packages/web/src/app/games/[gameId]/candidates/[candidateId]/page.tsx` の `generateMetadata` を改修する
    - `og:title`: `formatCandidateTitle(position)`
    - `og:description`: `truncateDescription(description, 100が
    - `og:image`: クエリパラメータ方式を廃止し、`buildOgpImageUrl(`/api/og/candidate/${candidateId}`)` を使用
    - `twitter:card`: `summary_large_image`（既存を維持）
    - _要件: 6.1, 6.2, 6.3, 6.4, 6.5_

- [x] 12. フロントエンド API クライアントの拡張
  - [x] 12.1 `packages/web/src/lib/api/client.ts` に特定ターンデータ取得関数を追加する
    - `fetchGameTurn(gameId: string, turnNumber: number): Promise<TurnResponse>` を実装
    - `TurnResponse` 型を `packages/web/src/types/game.ts` に追加
    - _要件: 2.1_

- [x] 13. チェックポイント - 全機能の統合検証
  - すべてのテストが pass することを確認し、ユーザーに質問があれば確認する。
