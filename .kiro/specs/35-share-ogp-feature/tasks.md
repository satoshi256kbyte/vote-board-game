...
- [-] 4. 特定ターン用バックエンド API の追加
  - [-] 4.1 `packages/api/src/routes/games.ts` に `GET /api/games/:gameId/turns/:turnNumber` エンドポイントを追加する
  ...
  - [~] 4.2 `packages/api/src/routes/games.test.ts` にターン API のユニットテストを追加する
    - 正常なターン取得、存在しない gameId、存在しないターン番号のテスト
    - _要件: 2.1, 2.3_

- [ ] 5. 特定ターン用 OGP 画像 API の新規作成
  - [~] 5.1 `packages/web/src/app/api/og/game/[gameId]/turn/[turnNumber]/route.tsx` を新規作成する
  ...
  - [~] 5.2 `packages/web/src/app/api/og/game/[gameId]/turn/[turnNumber]/route.test.tsx` にユニットテストを作成する
