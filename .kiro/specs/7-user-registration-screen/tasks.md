...
本実装計画は、投票対局アプリケーションのユーザー登録画面機能を実装するためのタスクリストです。この機能は、新規ユーザーがメールアドレスとパスワードを使用してアカウントを作成できるフロントエンド画面を提供します。実装は既存のユーザー登録API（spec 1）と統合し、Next.js 16、TypeScript、Tailwind CSS、shadcn/ui、Vitestを使用します。
...
- [ ] 6. RegisterPageコンポーネントの実装
  ...
  - [x] 6.2 RegisterPageコンポーネントのユニットテストを作成
...
- [ ] 7. プロパティベーステストの実装
  - [x] 7.1 プロパティ1のテストを作成: メールアドレス形式検証
