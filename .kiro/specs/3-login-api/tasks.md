...
投票対局アプリケーションのログインAPI機能を実装します。既存のユーザー登録APIのアーキテクチャを拡張し、`/auth/login`と`/auth/refresh`エンドポイントを追加します。
...
- [x] 5. Auth Routerへのエンドポイント追加
  ...
  - [x] 5.5 リフレッシュエンドポイントのユニットテストを作成
    - `packages/api/src/routes/auth.test.ts`に追加
    - 有効なリクエストの成功テスト（200）
    - バリデーションエラーのテスト（400）
