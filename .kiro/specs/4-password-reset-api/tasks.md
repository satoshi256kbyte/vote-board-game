...
- [ ] 1. バリデーションスキーマの追加
  - [ ] 1.1 passwordResetRequestSchemaとpasswordResetConfirmSchemaを作成
...
- [ ] 2. CognitoServiceの拡張
  - [ ] 2.1 forgotPasswordメソッドを追加
    ...
    - _要件: 2.1_
  - [ ] 2.2 confirmForgotPasswordメソッドを追加
    - `packages/api/src/lib/cognito/cognito-service.ts`に追加
    ...
    - _要件: 6.1_
  - [ ] 2.3 forgotPasswordとconfirmForgotPasswordのユニットテストを作成
...
- [ ] 3. RateLimiterのアクション別設定追加
  - [ ] 3.1 password-resetとpassword-reset-confirmのレート制限設定を追加
...
- [ ] 5. Auth Routerへのパスワードリセットエンドポイント追加
  ...
  - [ ] 5.3 エラー判定ヘルパー関数を実装
    ...
    - _要件: 2.3, 4.3, 8.1_
  - [ ] 5.4 パスワードリセット要求エンドポイントのユニットテストを作成
  ...
  - [ ] 5.5 パスワードリセット確認エンドポイントのユニットテストを作成
    - `packages/api/src/routes/auth.test.ts`に追加
    - 有効なリクエストの成功テスト（200）
    - バリデーションエラーのテスト（400 VALIDATION_ERROR）
  ...
  - [ ]\* 5.6 プロパティテスト: パスワードリセット要求の成功レスポンス
    - `packages/api/src/routes/auth.property.test.ts`に追加
    - **プロパティ2: パスワードリセット要求の成功レスポンス**
    - **検証: 要件 2.1, 2.2**
  - [ ]\* 5.7 プロパティテスト: アカウント列挙防止
    - `packages/api/src/routes/auth.property.test.ts`に追加
    - **プロパティ3: アカウント列挙防止**
    - **検証: 要件 2.3, 8.1**
  - [ ]\* 5.8 プロパティテスト: Cognito予期しないエラー時の500レスポンス
    - `packages/api/src/routes/auth.property.test.ts`に追加
    - **プロパティ4: Cognito予期しないエラー時の500レスポンス**
    - **検証: 要件 2.4, 6.3**
  - [ ]\* 5.9 プロパティテスト: 無効/期限切れ確認コードのエラーハンドリング
    - `packages/api/src/routes/auth.property.test.ts`に追加
    - **プロパティ6: 無効/期限切れ確認コードのエラーハンドリング**
    - **検証: 要件 4.3**
  - [ ]\* 5.10 プロパティテスト: パスワードリセット確認の成功レスポンス
    - `packages/api/src/routes/auth.property.test.ts`に追加
