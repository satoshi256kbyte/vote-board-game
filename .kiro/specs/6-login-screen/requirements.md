...
本ドキュメントは、投票対局アプリケーションのログイン画面機能の要件を定義します。ログイン画面は、登録済みユーザーがメールアドレスとパスワードを使用してアプリケーションにログインできるようにするフロントエンド機能です。この画面は、既存のログインAPI（`/auth/login`）と統合し、認証成功後にユーザーをアプリケーションのメイン画面に遷移させます。
