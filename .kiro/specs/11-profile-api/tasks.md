...
このタスクリストは、投票対局アプリケーションにおけるプロフィール関連APIの実装を段階的に進めるためのものです。実装は、バックエンドAPI（Hono + Lambda）、データレイヤー（DynamoDB）、S3サービス（Presigned URL生成）、バリデーション（Zod）、テスト（ユニットテスト + プロパティベーステスト）、インフラ（CDK）の順に進めます。
