...
/**
 * 投票変更API バリデーションスキーマのユニットテスト
 *
 * Requirements: 2.1, 2.2, 2.3, 2.4, 2.5
 */

describe('putVoteBodySchema', () => {
  describe('有効なリクエストボディ', () => {
    it('有効なUUID v4のcandidateIdを受け入れる', () => {
      ...
      expect(result.success).toBe(true);
      if (result.success) {
        expect(result.data.candidateId).toBe('890e1234-e89b-12d3-a456-426614174003');
      }
    });
  });
