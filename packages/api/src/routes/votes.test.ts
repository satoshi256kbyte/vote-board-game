...
/**
 * PUT /api/games/:gameId/turns/:turnNumber/votes/me Unit Tests
 *
 * Requirements: 1.1, 3.1, 3.2, 4.1, 4.2, 5.1, 6.1, 7.1, 9.1, 10.1, 10.2
 */
describe('PUT /api/games/:gameId/turns/:turnNumber/votes/me', () => {
  let app: Hono;
  ...
  describe('正常系 (200 OK)', () => {
    it('投票を変更して200を返す', async () => {
      const mockResponse: VoteChangeResponse = {
        gameId: validGameId,
        turnNumber: 5,
        userId,
        candidateId: validCandidateId,
      ...
      mockService.changeVote.mockResolvedValue(mockResponse);
      ...
      expect(mockService.changeVote).toHaveBeenCalledWith(validGameId, 5, validCandidateId, userId);
    });
  });
  ...
  describe('ゲーム未存在 (404)', () => {
    it('GameNotFoundErrorで404を返す', async () => {
      mockService.changeVote.mockRejectedValue(new GameNotFoundError(validGameId));
  ...
  describe('ターン未存在 (404)', () => {
    it('TurnNotFoundErrorで404を返す', async () => {
      mockService.changeVote.mockRejectedValue(new TurnNotFoundError(validGameId, 99));
  ...
  describe('候補未存在 (404)', () => {
    it('CandidateNotFoundErrorで404を返す', async () => {
      mockService.changeVote.mockRejectedValue(new CandidateNotFoundError(validCandidateId));
  ...
  describe('投票締切済み (400)', () => {
    it('VotingClosedErrorで400を返す', async () => {
      mockService.changeVote.mockRejectedValue(new VotingClosedError());
  ...
  describe('未投票 (409)', () => {
    it('NotVotedErrorで409を返す', async () => {
      mockService.changeVote.mockRejectedValue(new NotVotedError());
      ...
      expect(res.status).toBe(409);
      expect(data.error).toBe('NOT_VOTED');
      expect(data.message).toBe('Not voted yet in this turn');
    });
  });

  describe('同一候補 (400)', () => {
    it('SameCandidateErrorで400を返す', async () => {
      mockService.changeVote.mockRejectedValue(new SameCandidateError());
      ...
      expect(res.status).toBe(400);
      expect(data.error).toBe('SAME_CANDIDATE');
      expect(data.message).toBe('Already voting for this candidate');
    });
  });
