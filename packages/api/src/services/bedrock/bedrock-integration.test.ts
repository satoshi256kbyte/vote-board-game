...
describe('Bedrock Integration Tests', () => {
    ...
    describe('E2E: 初期化からAPI呼び出しまで', () => {
        it('should successfully initialize and call generateText', async () => {
            ...
            expect(console.log).toHaveBeenCalledWith(
                expect.stringContaining('BEDROCK_REQUEST')
            );
            expect(console.log).toHaveBeenCalledWith(
                expect.stringContaining('BEDROCK_TOKEN_USAGE')
            );
            expect(console.log).toHaveBeenCalledWith(
                expect.stringContaining('BEDROCK_SUCCESS')
            );
        });
    ...
    describe('E2E: エラーハンドリングの統合', () => {
        ...
        it('should handle validation errors without retry', async () => {
            ...
            // 空のプロンプトでバリデーションエラー
            await expect(
                bedrockService.generateText({ prompt: '' })
        ...
        it('should handle model not found errors', async () => {
            ...
            await expect(
                bedrockService.generateText({ prompt: 'Test model error' })
            ...
            // エラーログを検証
            expect(console.log).toHaveBeenCalledWith(
                expect.stringContaining('BEDROCK_ERROR')
            );
        });
        ...
        it('should exhaust retries and throw error', async () => {
            ...
            await expect(
                bedrockService.generateText({ prompt: 'Test timeout' })
    ...
    describe('E2E: トークンカウントとロギング', () => {
        ...
        it('should not log sensitive prompt data', async () => {
            ...
            // すべてのログエントリを確認
            const allLogs = (console.log as ReturnType<typeof vi.fn>).mock.calls.map(
                (call) => call[0]
            );
