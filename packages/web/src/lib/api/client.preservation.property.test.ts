...
/**
 * Property 11: Unauthenticated Endpoints Work Without Token
 * **Validates: Requirements 3.1, 3.4 (Preservation of public endpoints)**
 *
 * For any public endpoint (fetchGames, fetchGame, fetchCandidates GET),
 * the API client should successfully make requests without requiring
 * an authentication token, maintaining backward compatibility.
 */
it('Property 11: 認証不要なエンドポイント（fetchGames, fetchGame）はトークンなしで動作する', async () => {
  // Mock storageService to return null (no token)
  ...
  await fc.assert(
    fc.asyncProperty(
      fc.constantFrom('fetchGames', 'fetchGame'),
      fc.uuid(),
      async (operation, gameId) => {
        mockFetch.mockClear();
